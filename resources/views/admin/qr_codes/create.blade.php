<x-admin-layout>
    ...
    <div class="max-w-2xl bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        <form action="{{ route('admin.qr_codes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Account Name / Label</label>
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g., TNG eWallet / Shop QR" 
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">QR Code Image</label>
                <input type="file" name="image" required accept="image/*"
                       class="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-500 transition font-medium">
                <p class="text-xs text-slate-400 mt-2 italic">Recommended: Clear, high-resolution QR image.</p>
